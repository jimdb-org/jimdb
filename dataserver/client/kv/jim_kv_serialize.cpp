...
JimStatus del_encode(KvCmdPtr cmd, const ArrayString &args) {
    ...
    uint16_t key_hash = jimkv_hash(key);
    //set encode_key
    std::string encode_key = key_encode(cmd->table_id(), key);

    //del command, don't encode hash code & type
    //encode_uint16(key_hash, encode_key);
    cmd->set_hash_code(key_hash);

    cmd->set_scope_key(std::move(JimScope(encode_key)));
    cmd->set_origin_key(key);

    dspb::RangeRequest req;
    auto fn = [](dspb::RangeRequest &req, JimScope &scope) {
        auto kv_del_range = req.mutable_kv_del_range();

        kv_del_range->set_start_key(scope.start());
        kv_del_range->set_end_key(scope.end());
    };

    fn(req, cmd->scope_key());
    cmd->set_range_request(std::move(req), fn);
...
JimStatus get_encode(KvCmdPtr cmd, const ArrayString &args) {
    ...
    cmd->set_key(encode_key);
    cmd->set_origin_key(key);

    dspb::RangeRequest req;
    auto kv_get = req.mutable_kv_get();

    kv_get->set_key(encode_key);

    cmd->set_range_request(std::move(req));
...
JimStatus set_encode_ex(KvCmdPtr cmd, const ArrayString &args, uint64_t expire) {
    ...
    dspb::RangeRequest req;
    auto kv_put = req.mutable_kv_put();

    kv_put->set_key(encode_key);
    kv_put->set_value(encode_value);
...
JimStatus hdel_encode(KvCmdPtr cmd, const ArrayString &args) {
    ...
    dspb::RangeRequest req;
    auto kv_delete = req.mutable_kv_delete();

    kv_delete->set_key(encode_key);
...
JimStatus hgetall_encode(KvCmdPtr cmd, const ArrayString &args) {
    ...
    dspb::RangeRequest req;
    auto fn = [](dspb::RangeRequest &req, JimScope &scope) {
        auto kv_scan = req.mutable_kv_scan();

        kv_scan->set_start_key(scope.start());
        kv_scan->set_end_key(scope.end());
    };
