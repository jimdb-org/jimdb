...
void kv_string_test(KvStringTest &test) {
    CBLOG_INFO("test SET");
    test.set(std::string("jim_kv_string"), std::string("jim_kv_value"));

    CBLOG_INFO("test GET");
    auto v = test.get(std::string("jim_kv_string"));
    print_reply(v);
    ...
    auto v2 = test.strlen(std::string("jim_kv_string"));
    print_reply(v2);

    CBLOG_INFO("test EXISTS");
    auto v3 = test.exists(std::string("jim_kv_string"));
    print_reply(v3);

    CBLOG_INFO("test SET EX");
    test.set(std::string("jim_kv_string"), std::string("jim_kv_value"), 1, 1000);

    CBLOG_INFO("test TTL");
    auto t1 = test.ttl(std::string("jim_kv_string"));
    print_reply(t1);

    CBLOG_INFO("test PTTL");
    auto t2 = test.pttl(std::string("jim_kv_string"));
    print_reply(t2);

    CBLOG_INFO("test SETEX");
    test.setex(std::string("jim_kv_string"), std::string("jim_kv_value"), 3000);

    CBLOG_INFO("test TTL");
    auto t3 = test.ttl(std::string("jim_kv_string"));
    print_reply(t3);

    CBLOG_INFO("test PSETEX");
    test.psetex(std::string("jim_kv_string"), std::string("jim_kv_value"), 60000000);

    CBLOG_INFO("test PTTL");
    auto t4 = test.ttl(std::string("jim_kv_string"));
    print_reply(t4);
    ...
    ArrayString keys = {"jim_kv_mk_1", "jim_kv_mk_3","jim_kv_mk_5","jim_kv_mk_9"};
...
void kv_hash_test(KvHashTest &test) {
    ...
    CBLOG_INFO("test EXISTS");
    auto v5 = test.exists(std::string("jim_kv_hash"));
    print_reply(v5);
