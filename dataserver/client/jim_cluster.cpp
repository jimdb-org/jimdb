...
bool JimCluster::SendSubCmd(CmdPtr cmd, asio::steady_timer::time_point expire) {
    if (cmd->key().empty()) {
        ...
        return SplitScope(msg, cmd->scope_key());
    }
    ...
    auto req = cmd->range_request();

    auto head = req.mutable_header();
    head->set_cluster_id(cmd->cluster_id());
    head->set_range_id(range->id());
    head->mutable_range_epoch()->CopyFrom(range->range_epoch());

    if (msg->Serialize(req)) {
        msg->expire_at_set(expire);
...
bool JimCluster::SendSubMsg(MsgPtr msg, JimScope& scope, RangePtr rng) {
    ...
    auto req = msg->cmd()->range_request(scope);

    auto head = req.mutable_header();
    head->set_cluster_id(msg->cmd()->cluster_id());
    head->set_range_id(rng->id());
    head->mutable_range_epoch()->CopyFrom(rng->range_epoch());

    if (!sub_msg->Serialize(req)) {
...
bool JimCluster::SplitScope(MsgPtr msg, const JimScope& scope) {
    ...
    while (cmd->valid()) {
        ...
        if (range == nullptr) {
            CBLOG_WARN("not found route (key):%s", EncodeToOct(start_key).c_str());
...
bool JimCluster::ReSendMsg(MsgPtr msg) {
    if (!msg->has_parent()) {
        if (msg->timeout()) {
            ...
            CBLOG_INFO("resend msg key: %s", EncodeToOct(msg->cmd()->key()).c_str());
    ...
    JimScope scope = msg->scope_key();
    CBLOG_INFO("resend msg key: [%s -%s)",
        EncodeToOct(scope.start()).c_str(), EncodeToOct(scope.end()).c_str());
