...
protected:
    ...
    void SetUp() override {
        char path[] = "/tmp/jim_ds_meta_store_test_XXXXXX";
    ...
    void TearDown() override {
        delete store_;
        if (!tmp_dir_.empty()) {
            jim::RemoveDirAll(tmp_dir_.c_str());
        }
    }
...
TEST_F(MetaStoreTest, ApplyIndex) {
    uint64_t range_id = jim::randomInt();
    uint64_t applied = 1;
    auto s = store_->LoadApplyIndex(range_id, &applied);
    ASSERT_TRUE(s.ok()) << s.ToString();
    ASSERT_EQ(applied, 0U);

    uint64_t save_applied = jim::randomInt();
