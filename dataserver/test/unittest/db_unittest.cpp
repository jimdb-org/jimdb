...
protected:
    ...
    void SetUp() override {
        char path[] = "/tmp/jim_ds_db_test_XXXXXX";
        char *tmp = mkdtemp(path);
        ASSERT_TRUE(tmp != NULL);
        data_path_ = tmp;

        auto s = jim::test::helper::NewDBManager(data_path_, manager_);
        ASSERT_TRUE(s.ok()) << s.ToString();
    ...
    void TearDown() override {
        ...
        manager_.reset();
        if (!data_path_.empty()) {
            jim::RemoveDirAll(data_path_.c_str());
        }
    }
    ...
    void ReOpenDB() {
        ...
        s = jim::test::helper::NewDBManager(data_path_, manager_);
        ASSERT_TRUE(s.ok()) << s.ToString();
...
TEST_F(DBTest, PutGetDelete) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (int j = 0; j < 1000; ++j) {
            std::string key = jim::randomString(32, 1024);
            std::string value = jim::randomString(64);
