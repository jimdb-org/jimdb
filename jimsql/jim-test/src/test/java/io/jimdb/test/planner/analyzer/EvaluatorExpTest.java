 ...
 */
package io.jimdb.test.planner.analyzer;
...
public class EvaluatorExpTest extends TestBase {
  ...
  public void init() {
    //mock tables
//    setMetaEngine(new MockMetaEngine4Expr());
  }
  ...
  public List<Checker> arithmeticVarTestCase() {
    ...
    checkerList.add(EvaluatorExpChecker.build()
            ...
            .addCheckPoint(CheckPoint.EvaluatorExp, "PlusDecimal(CastIntToDecimal(test.user.age),1)"));
    checkerList.add(EvaluatorExpChecker.build()
            .sql("select pas+1.0 from user ")
            .addCheckPoint(CheckPoint.EvaluatorExp, "PlusReal(test.user.pas,CastDecimalToReal(1))"));
    checkerList.add(EvaluatorExpChecker.build()
            .sql("select pas+1 from user ")
            .addCheckPoint(CheckPoint.EvaluatorExp, "PlusReal(test.user.pas,1.0)"));

    checkerList.add(EvaluatorExpChecker.build()
            .sql("select scale+1 from user ")
            .addCheckPoint(CheckPoint.EvaluatorExp, "PlusDecimal(test.user.scale,1)"));
    checkerList.add(EvaluatorExpChecker.build()
            .sql("select scale+1.0 from user ")
            .addCheckPoint(CheckPoint.EvaluatorExp, "PlusDecimal(test.user.scale,1)"));
    checkerList.add(EvaluatorExpChecker.build()
            .sql("select scale+'1.0' from user ")
            .addCheckPoint(CheckPoint.EvaluatorExp, "PlusReal(CastDecimalToReal(test.user.scale),1.0)"));
    return checkerList;
  }
