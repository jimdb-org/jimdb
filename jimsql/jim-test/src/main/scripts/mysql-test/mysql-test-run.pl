...
sub command_line_setup {
    ...
    if ($path_client_bindir) {
        ...
        $path_client_bindir = "$glob_mysql_test_dir/bin";
    }
    ...
    if (defined $ENV{'MYSQL_TEST'}) {
        $exe_mysqltest = mtr_exe_exists($ENV{'MYSQL_TEST'});
    }
    elsif (IS_LINUX) {
        $exe_mysqltest = mtr_exe_exists("$glob_mysql_test_dir/bin/mysqltest_linux");
    }
    elsif (IS_MAC) {
        $exe_mysqltest = mtr_exe_exists("$glob_mysql_test_dir/bin/mysqltest_mac");
    }
    else {
        mtr_error("Could not find 'mysqltest'");
    }
    ...
    if (using_extern()) {
        ...
        mtr_add_arg($args, "--max-connect-retries=3");
        while (my ($option, $value) = each(%opts_extern)) {
            mtr_add_arg($args, "--$option=$value");
        }
        mtr_add_arg($args, "--silent");
        mtr_add_arg($args, "--test-file=t/ping.test");
        my $cmd = "$exe_mysqltest " . join(' ', @$args);
        mtr_verbose("cmd: $cmd");
        my $list = `$cmd` or
        ...
        mtr_add_arg($args, "--port=3360");
        mtr_add_arg($args, "--user=%s", $opt_user);
        mtr_add_arg($args, "--password=root");
        mtr_add_arg($args, "--silent");
        mtr_add_arg($args, "--test-file=t/ping.test");
        my $cmd = "$exe_mysqltest " . join(' ', @$args);
        mtr_verbose("cmd: $cmd");
...
sub run_server() {
    my $dir_sql = dirname($glob_mysql_test_dir);
    system "cd $dir_sql/bin;sh start.sh";
    sleep(20);
}
