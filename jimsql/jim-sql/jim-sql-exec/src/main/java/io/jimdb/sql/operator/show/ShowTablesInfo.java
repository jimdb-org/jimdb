 ...
 * @version V1.0
 */
public final class ShowTablesInfo extends RelOperator {
  private final String database;

  public ShowTablesInfo(String database) {
  ...
  public Flux<ExecResult> execute(Session session) throws BaseException {
    ...
    for (Table table : tables) {
      Value[] values = new Value[]{
              LongValue.getInstance(table.getCatalogId()),
              LongValue.getInstance(table.getId()),
