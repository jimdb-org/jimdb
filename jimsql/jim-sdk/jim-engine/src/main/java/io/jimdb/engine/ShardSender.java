...
public final class ShardSender implements Closeable {
  ...
  private final Router routeManager;
  private final NettyClient client;
  private final ExecutorService retryThreadPool;
  private final ScheduledExecutorService retryExecutor;

  public ShardSender(final JimConfig config, final Router routeManager) {
    NettyClientConfig clientConfig = config.getClientConfig();
    Preconditions.checkArgument(clientConfig != null, "client config cannot be null");
    ...
    Preconditions.checkArgument(retryThreadPool != null, "retryThreadPool cannot be null");
  ...
  private void sendCommand(final RequestContext context, final CommandCallback<JimCommand> callback) {
    try {
      ...
      Message msgBody = buildRangeRequest(context, rangeInfo, context.getReqBuilder());
  ...
  private void retryCommand(final RequestContext context, final CommandCallback<JimCommand> callback, Throwable err) {
    retryThreadPool.execute(() -> {
      ...
      if (delay > 0) {
        LOG.warn("request with cxtId {} has not been timed out, cmdType:{}, instant: {}, retry, err{}", context.getCtxId(),
            context.getCmdType(), context.getTimeOut(), err);
...
//        case ER_META_SHARD_NOT_EXIST:
  ...
  private void clearRoute(RequestContext context) {
    RoutingPolicy routingPolicy = context.getRoutePolicy();
    if (routingPolicy != null) {
      routingPolicy.removeRangeByKey(context.getKey().toByteArray(), null);
    }
  }
  ...
  private static Message buildRangeRequest(RequestContext context, RangeInfo range, Message.Builder reqBuilder) {

    Api.RangeRequest.Header.Builder headerBuilder = Api.RangeRequest.Header.newBuilder();
    headerBuilder.setClusterId(context.getClusterId());
    headerBuilder.setRangeId(range.getId());
    headerBuilder.setRangeEpoch(range.getRangeEpoch());

    Api.RangeRequest.Builder requestBuilder = Api.RangeRequest.newBuilder().setHeader(headerBuilder);

    ReqCase cmdType = context.getCmdType();
    switch (cmdType) {
      case KV_GET:
        requestBuilder.setKvGet((Kv.KvGetRequest.Builder) reqBuilder);
        break;
      case KV_PUT:
        requestBuilder.setKvPut((Kv.KvPutRequest.Builder) reqBuilder);
        break;
      case KV_DELETE:
        requestBuilder.setKvDelete((Kv.KvDeleteRequest.Builder) reqBuilder);
        break;
      case PREPARE:
        requestBuilder.setPrepare((Txn.PrepareRequest.Builder) reqBuilder);
        break;
      case DECIDE:
        requestBuilder.setDecide((Txn.DecideRequest.Builder) reqBuilder);
        break;
      case CLEAR_UP:
        requestBuilder.setClearUp((Txn.ClearupRequest.Builder) reqBuilder);
        break;
      case GET_LOCK_INFO:
        requestBuilder.setGetLockInfo((Txn.GetLockInfoRequest.Builder) reqBuilder);
        break;
      case SELECT:
        requestBuilder.setSelect((Txn.SelectRequest.Builder) reqBuilder);
        break;
      case SCAN:
        requestBuilder.setScan((Txn.ScanRequest.Builder) reqBuilder);
        break;
      case SELECT_FLOW:
        requestBuilder.setSelectFlow((Txn.SelectFlowRequest.Builder) reqBuilder);
        break;
      case INDEX_STATS:
        requestBuilder.setIndexStats((Statspb.IndexStatsRequest.Builder) reqBuilder);
        break;
      case COLUMNS_STATS:
        requestBuilder.setColumnsStats((Statspb.ColumnsStatsRequest.Builder) reqBuilder);
        break;
      default:
