...
public class MultiplyFunc extends Func {
  ...
  protected MultiplyFunc(Session session, Exprpb.ExprType code, Expression[] args, ValueType retTp,
  ...
  public LongValue execLong(ValueAccessor accessor) throws JimException {
    ...
    return ValueConvertor.convertToLong(session, v1.multiply(session, v2), null);
  }
  ...
  public UnsignedLongValue execUnsignedLong(ValueAccessor accessor) throws JimException {
    ...
    return (UnsignedLongValue) v1.multiply(session, v2);
  }
  ...
  public DoubleValue execDouble(ValueAccessor accessor) throws JimException {
    ...
    return (DoubleValue) v1.multiply(session, v2);
  }
  ...
  public DecimalValue execDecimal(ValueAccessor accessor) throws JimException {
    ...
    return (DecimalValue) v1.multiply(session, v2);
  }

  /**
   *
   */
  public static class MultiplyFuncBuilder extends BinaryFuncBuilder {
    public MultiplyFuncBuilder(String name) {
      super(name);
    }
    ...
    protected Func doBuild(Session session, Expression[] args) {
      ...
      if (arg1ValueType == ValueType.DOUBLE || arg2ValueType == ValueType.DOUBLE) {
        MultiplyFunc result = new MultiplyFunc(session, Exprpb.ExprType.MultReal, args, ValueType.DOUBLE,
      ...
      if (arg1ValueType == ValueType.DECIMAL || arg2ValueType == ValueType.DECIMAL) {
        MultiplyFunc result = new MultiplyFunc(session, Exprpb.ExprType.MultDecimal, args, ValueType.DECIMAL,
      ...
      if (arg1Type.getUnsigned() || arg2Type.getUnsigned()) {
        result = new MultiplyFunc(session, Exprpb.ExprType.MultInt, args, ValueType.UNSIGNEDLONG,
                ValueType.UNSIGNEDLONG, ValueType.UNSIGNEDLONG);
      } else {
        result = new MultiplyFunc(session, Exprpb.ExprType.MultInt, args, ValueType.LONG, ValueType.LONG,
                ValueType.LONG);
      }
