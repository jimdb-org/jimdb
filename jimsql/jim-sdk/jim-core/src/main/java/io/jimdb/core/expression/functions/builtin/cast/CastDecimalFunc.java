 ...
 * @version V1.0
 */
final class CastDecimalFunc extends Func {
  private CastDecimalFunc() {
  }

  protected CastDecimalFunc(Exprpb.ExprType code, Expression[] args, SQLType resultType, Session session) {
    super();
    this.code = code;
    this.name = code.name();
    this.args = args;
  ...
  @Override
  public CastDecimalFunc clone() {
    CastDecimalFunc result = new CastDecimalFunc();
    clone(result);
    return result;
  }
