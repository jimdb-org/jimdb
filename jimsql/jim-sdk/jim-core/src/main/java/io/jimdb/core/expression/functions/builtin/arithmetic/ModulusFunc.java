...
public class ModulusFunc extends Func {
  public ModulusFunc() {
  }

  public ModulusFunc(Session session, Exprpb.ExprType code, Expression[] args, ValueType retTp,
  ...
  @Override
  public Func clone() {
    ModulusFunc modulusFunc = new ModulusFunc();
    clone(modulusFunc);
    return modulusFunc;
  }
  ...
  public LongValue execLong(ValueAccessor accessor) throws BaseException {
    ...
    return ValueConvertor.convertToLong(session, v1.mod(session, v2), null);
  }
   ...
   *
   */
  public static class ModFuncBuilder extends BinaryFuncBuilder {

    public ModFuncBuilder(String name) {
      super(name);
    }
    ...
    protected Func doBuild(Session session, Expression[] args) {
      ...
      if (arg1ValueType == ValueType.DOUBLE || arg2ValueType == ValueType.DOUBLE) {
        ModulusFunc result = new ModulusFunc(session, Exprpb.ExprType.ModReal, args, ValueType.DOUBLE,
      ...
      if (arg1ValueType == ValueType.DECIMAL || arg2ValueType == ValueType.DECIMAL) {
        ModulusFunc result = new ModulusFunc(session, Exprpb.ExprType.ModDecimal, args, ValueType.DECIMAL,
      ...
      if (arg1Type.getUnsigned() || arg2Type.getUnsigned()) {
        result = new ModulusFunc(session, Exprpb.ExprType.ModInt, args, ValueType.UNSIGNEDLONG,
                ValueType.UNSIGNEDLONG, ValueType.UNSIGNEDLONG);
      } else {
        result = new ModulusFunc(session, Exprpb.ExprType.ModInt, args, ValueType.LONG, ValueType.LONG, ValueType.LONG);
      }
