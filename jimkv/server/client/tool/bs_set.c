...
#define SET_WRITE_CMD_SIZE  3
#define SET_READ_CMD_SIZE   5

static bs_command_call_t write_commands[SET_WRITE_CMD_SIZE];
static bs_command_call_t read_commands[SET_READ_CMD_SIZE];
...
void set_command(char *key, unsigned int *seed, int key_code, int mode) {
    ...
    if (mode == BS_WRITE_COMMAND) {
        call = write_commands[hash_code % SET_WRITE_CMD_SIZE];
    } else {
        call = read_commands[hash_code % SET_READ_CMD_SIZE];
    }
...
*/
static void rds_sscan(char *key, unsigned int *seed) {
