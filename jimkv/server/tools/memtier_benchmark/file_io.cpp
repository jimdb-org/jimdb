...
memcache_item* file_reader::read_item(void)
{
    ...
    // scan int values
    if ((ret = fscanf(m_file, "%u, %u, %u, %u, %u, %u, %u, %u, ",
        &s_dumpflags,
        &s_time,
        &s_exptime,
        &s_nbytes,
        ...
        fprintf(stderr, "%s:%u: error parsing item values., s_dumpflags: %u, s_time: %u, s_exptime: %u, s_nbytes: %u, s_nsuffix: %u,s_flags: %u, s_clsid: %u, s_nkey: %u\n", m_filename, m_line,s_dumpflags, s_time, s_exptime, s_nbytes, s_nsuffix, s_flags, s_clsid, s_nkey);
        return NULL;
    }

        //fprintf(stderr, "item values s_dumpflags: %u, s_time: %u, s_exptime: %u, s_nbytes: %u, s_nsuffix: %u,s_flags: %u, s_clsid: %u, s_nkey: %u\n", s_dumpflags, s_time, s_exptime, s_nbytes, s_nsuffix, s_flags, s_clsid, s_nkey);
    // read key
