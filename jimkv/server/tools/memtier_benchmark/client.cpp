...
bool client::setup_client(benchmark_config *config, abstract_protocol *protocol, object_generator *objgen)
{
    ...
    if (!strcmp(config->protocol, "cbdb")) {
        conn = new cbdb_connection(m_connections.size(), this, m_config, m_event_base, protocol);
    } else {
        conn = new shard_connection(m_connections.size(), this, m_config, m_event_base, protocol);
    }
