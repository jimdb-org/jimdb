...
void cluster_client::disconnect(void)
{
    ...
    for (i = 0; i < m_connections.size(); i++) {
        connection* sc = m_connections[i];
        sc->disconnect();
    }

    // delete all connections except main connection
    for (i = conn_size - 1; i > 0; i--) {
        connection* sc = m_connections.back();
        m_connections.pop_back();
        delete sc;
    }
}

connection* cluster_client::create_connection(abstract_protocol* abs_protocol) {
    connection* sc = new shard_connection(m_connections.size(), this,
...
bool cluster_client::connect_connection(connection* sc, char* address, char* port) {
...
void cluster_client::handle_cluster_slots(protocol_response *r) {
    ...
    for (unsigned int i=0; i<r->get_mbulk_value()->mbulks_elements.size(); i++) {
        ...
        for (j = 0; j < m_connections.size(); j++) {
            if (strcmp(addr, m_connections[j]->get_address()) == 0 &&
                ...
                if (sc->get_connection_state() == conn_disconnected) {
                    connect_connection(sc, addr, port);
                }
        ...
        if (sc == NULL) {
            sc = create_connection(MAIN_CONNECTION->get_protocol());
            connect_connection(sc, addr, port);
        }
