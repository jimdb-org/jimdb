...
void make_server_config()
{
    ...
    std::string path = "/tmp/jimkv_store_test_";
...
bool make_data(const basepb::TableInfo & t, std::unique_ptr<jimkv::ds::storage::Store>& store )
{
...
int main(int argc, char **argv) {
    ...
    std::unique_ptr<jimkv::ds::storage::Store> store = nullptr;

    auto s = jimkv::ds::db::NewDBManager(db_manager);
    if (!s.ok()) {
        std::cout << s.ToString() << std::endl;
        return -1;
    }
    ...
    store.reset(new jimkv::ds::storage::Store(r, std::move(db)));
