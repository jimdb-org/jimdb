...
using jimkv::randomInt;
using jimkv::randomString;

Peer RandomPeer() {
    Peer p;
    p.type = (randomInt() % 2 == 0 ? jimkv::raft::PeerType::kLearner
                                   : jimkv::raft::PeerType::kNormal);
    p.node_id = randomInt();
    p.peer_id = randomInt();
    return p;
}
...
EntryPtr RandomEntry(uint64_t index, int data_size) {
    EntryPtr e(new jimkv::raft::impl::pb::Entry);
    e->set_index(index);
    e->set_term(randomInt());
    e->set_type((randomInt() % 2 == 0) ? jimkv::raft::impl::pb::ENTRY_NORMAL
                                       : jimkv::raft::impl::pb::ENTRY_CONF_CHANGE);
